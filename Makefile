...
$(BUILD_NINJA):
	cmake --preset default -DCMAKE_COLOR_DIAGNOSTICS=ON -DCMAKE_BUILD_TYPE=$(CONFIG) $(if $(LIBNEO_GIT_TAG),-DLIBNEO_GIT_TAG=$(LIBNEO_GIT_TAG)) $(if $(LIBNEO_SOURCE_DIR),-DLIBNEO_SOURCE_DIR=$(LIBNEO_SOURCE_DIR)) $(if $(FGSL_PATH),-DFGSL_PATH=$(FGSL_PATH)) $(if $(PYTHON_EXECUTABLE),-DPython_EXECUTABLE=$(PYTHON_EXECUTABLE))
