\chapter{Building}
For basic information and some more see the file \file{Build.md} in the
\neotwo main directory.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Dependencies}
This section aims at listing the dependencies, and giving some
background on where there are used and how they are included in the
code.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{fortnum}
The numerical backend is the fortnum library, fetched together with
libneo. It provides the numerical integration, special functions and
B-splines used by \neotwo. It replaces the former GSL/FGSL dependency,
which is no longer required.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{SuperLu}
Was used by sparse\_solver if \inputparameter{sparse\_solve\_method} is $1$.
This code branch has been removed in December 2024.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{SuiteSparse}
Is used by sparse\_solver if \inputparameter{sparse\_solve\_method} is
$2$ or $3$.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{ATLAS}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{OpenBLAS}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{MPI/MyMPILib}
Used for parallelization. In some places also OpenMP is used.
Now included directly in NEO-2.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{HDF5/hdf5\_tools}
Used for input and output.
Now included in libneo.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{libneo}
Collection for common code of the work group.
Provides shared field and HDF5 functionality used by NEO-2.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{f90nml}
Python module, used to read and edit fortran namelist files.
If not present on your system you can install it with
\begin{verbatim}
pip3 install --local f90nml
\end{verbatim}
