...
\begin{itemize}
	\item \vv{inp_swi}: \vv{NEO-2-QL} as well as \vv{NEO-2-PAR} get their magnetic 
     ...
     Boozer coordinates. (see the relevant sections on \href{https://gitlab.tugraz.at/plasma/info/}{INFO} 
     for more details on Boozer files and magnetic field formats). There 
	 is no way to sugarcoat this, but due to non-uniform formating of the 
	...
	\begin{itemize}
		...
		in total if you include the modenumber columns). See again the relevant
        section on \href{https://gitlab.tugraz.at/plasma/info/-/blob/GeorgGrassler/TOPIC_MHD_Equilibria/Fileformats.md}{INFO}.
	...
	toroidal $n$ modes for the 0th poloidal mode $m=0$ (see \href{https://gitlab.tugraz.at/plasma/info/-/blob/GeorgGrassler/TOPIC_MHD_Equilibria/Fileformats.md}{INFO}
    for the definitions of the Fourier series represented in the Boozer files).
	...
	but
	\begin{equation*}
		 f^\text{s}_{0n} = f^\text{c}_{0n} = 0 \quad\text{for } n<0 \rightarrow \text{left out in Boozer file}
	\end{equation*}
...
See \vv{https://gitlab.tugraz.at/plasma/info} for further information about 
concepts mentioned in this documentation (e.g. Boozer files).
