...
\author[1]{Rico~Buchholz}\author[1]{Georg Graßler}
...
\begin{itemize}
	\item \vv{inp_swi}: NEO-2 (QL as well as PAR version) get the magnetic
	 field data provided to them by so called Boozer files. Boozer files 
	 (marked usually with the ending \vv{.bc}) are basically text files 
	 written in a specific format containing the magnetic field data as 
	 fouriermodes in Boozer coordinates (see \vv{https://gitlab.tugraz.at/plasma/info} 
	 for more details on Boozer files and magnetic field formats). There 
	 is no way to sugarcoat this, but due to no uniform formating of the 
	 different Boozer files, many read-in options for the files exist in 
	 NEO-2. While the choice of \vv{inp_swi} seems therefore difficult, 
	 in certain case the choice is actually quite limited and clear. 
	 For an NTV calculation NEO-2 has to read in an axissymmetric field 
	 and a perturbation field. Internally it is hard coded that the same 
	 \vv{inp_switch} is used for both files, but for the read in of the 
	 perturbation field, only \vv{inp_swi=8 or 9} are available. The 
	 difference between the two options is
	
	\begin{itemize}
		\item \vv{inp_swi=8} assumes stellerator symmetry. This means that 
		for example for the $R$ coordinate not both the sine and cosine 
		fouriercoefficients are given, but only the cosine one (as the sine 
		...
		in total if you include the modenumber columns).
		\item \vv{inp_swi=9} does not assume stellerator symmetry, therefore 
		expecting 8 coefficient colummns. 
	\end{itemize}

	TD;LR: Check the number of columns in your input \vv{.bc} file and choose 
	one of the two choices above accordingly. For the NEO-QL, which deals with tokamaks,
	...
	convention in the listing of the fouriermodes, where there are no negative 
	toroidal $n$ modes for the 0th poloidal mode $m=0$. This would usually 
	lead to a crash of the reader as it would expect $(m_\text{max} + 1)*(2*n_\text{max} + 1)$ 
	modes, while actually due to the missing negative toroidal modes it is 
	$n_\text{max}$ less than that. However, as an axissymmetric field has $0$ 
	toroidal modes, the discrepancy is $0$ as well and the code "runs without 
...
$\sqrt{s_\text{pol}}$ is commonly used in experiemental context and therefore 
measured profile data. Furthermore, $\sqrt{s_\text{pol}}$ is the used input for 
the MARS code from the OMFIT library. For that reason, during generation of the 
NEO-2 profile input, \vv{generate_neo2_profile} makes a mapping 
$\sqrt{s_\text{pol}} \rightarrow \sqrt{s_\text{tor}}$ and then saves the profile 
in terms of $s = \sqrt{s_\text{tor}}^2$, which is the adequat input for NEO-2.
