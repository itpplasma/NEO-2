...
Here \vv{hdf5tools} still comes from \vv{libneo}, while
\vv{MyMPILib} lives directly in the \vv{NEO-2} repository. That keeps
the MPI support code with the codebase that actually uses it.
...
\subsubsection{Splitting of axisymmetric and perturbation field}
The above python script does not only perform a transformation into Boozer Coordinates,
but subsequently also splits this Boozer spectrum into an axisymmetric and perturbation part.
Specifically, it generates three kinds of files

\begin{enumerate}
	\item \vv{wout_test.bc} - the total Boozer spectrum of the field
	\item \vv{wout_test-n0.bc} - the axisymmetric part of the spectrum ($m=0, \ldots, +m_\text{max}$, $n=0$)
	\item \vv{wout_test-n*.bc} - the $n$-mode perturbation extracted from the spectrum, one file per specific $n$ ($m=-m_\text{max}, \ldots, +m_\text{max}$, $n=*$)
\end{enumerate}

\vv{NEO-2-QL} needs the latter two (the axisymetric part and \textbf{one} selected $n$-mode of the perturbation) for the NTV calculation.

Depending on how the original total Boozer spectrum of the field is given, a mapping has to be performed for the extracted data to be suitable for \vv{NEO-2}. \vv{NEO-2-QL} expects a spectrum of the form

\begin{equation}
	f = \sum_{m=-\infty}^{\infty} \sum_{n=0}^{\infty} \hat{f}^c_{mn}\cos{(m\theta_B - n\varphi_B)} + \hat{f}^s_{mn}\sin{(m\theta_B - n\varphi_B)} \label{eqFourierNhalfside}
\end{equation}

to be split into

\begin{equation}
	f_0 = \sum_{m=-\infty}^{\infty} \hat{f}^c_{m0}\cos{(m\theta_B - n\varphi_B)} + \hat{f}^s_{m0}\sin{(m\theta_B - n\varphi_B)}
\end{equation}
\begin{equation}
	f_n = \sum_{m=-\infty}^{\infty} \hat{f}^c_{mn}\cos{(m\theta_B - n\varphi_B)} + \hat{f}^s_{mn}\sin{(m\theta_B - n\varphi_B)} \text{ for } n = 0,\ldots, \infty 
\end{equation}

If the spectrum is given in the form~\ref{eqFourierNhalfside}, this can be done directly by taking the individual coefficients of the total spectrum, saving them into separate files. However, often the spectrum is given as

\begin{equation}
	f = \sum_{m=0}^{\infty} \sum_{n=-\infty}^{\infty} \tilde{f}^c_{mn}\cos{(m\theta_B - n\varphi_B)} + \tilde{f}^s_{mn}\sin{(m\theta_B - n\varphi_B)},
\end{equation}

where now the toroidal mode number goes from $-\infty$ to $\infty$, while the poloidal mode number only covers non-negative values. The corresponding coefficients $\tilde{f}_{mn}$ are therefore different compared to $\hat{f}_{mn}$. A mapping between these coefficients can be found by considering the full Fourier series these two half-side representations are based on:

\begin{equation}
	f = \sum_{m=-\infty}^{\infty} \sum_{n=-\infty}^{\infty} f^c_{mn}\cos{(m\theta_B - n\varphi_B)} + f^s_{mn}\sin{(m\theta_B - n\varphi_B)}.
\end{equation}

The coefficients fulfill the following conditions:

\begin{enumerate}
	\item $f^c_{mn} = f^c_{-m-n}$
	\item $f^s_{mn} = -f^s_{-m-n}$
\end{enumerate}

These conditions follow from the complex Fourier representation of a real valued function.
One can now combine terms of the sums together, choosing to half the summation of one of the mode numbers:

\begin{equation*}
	\tilde{f}^c_{mn} \equiv f^c_{mn} + f^c_{-m-n} \text{ for } m \geq 0 , n \in \mathbb{Z}
\end{equation*}
\begin{equation*}
	\tilde{f}^s_{mn} \equiv f^s_{mn} - f^s_{-m-n} \text{ for } m \geq 0 , n \in \mathbb{Z}
\end{equation*}

or

\begin{equation*}
	\hat{f}^c_{mn} \equiv f^c_{mn} + f^c_{-m-n} \text{ for } n \geq 0 , m \in \mathbb{Z}
\end{equation*}
\begin{equation*}
	\hat{f}^s_{mn} \equiv f^s_{mn} - f^s_{-m-n} \text{ for } n \geq 0 , m \in \mathbb{Z}
\end{equation*}

The definition of the coefficients are formally the same. However, note that there are no $\tilde{f}_{mn}$ for negative $m$. For the desired representation~\ref{eqFourierNhalfside} one needs coefficients with $m<0$ as well. The coefficients $\hat{f}^s_{mn}$ still formally fulfill the symmetry/anti-symmetry conditions of the original coefficients (like $\hat{f}^s_{mn} = -\hat{f}^s_{-m-n}$). This one can use to get a mapping for the necessary $\hat{f}_{mn}$ from the $\tilde{f}_{mn}$. In the following, $m_+$ notes a non-negative toroidal mode number (including $m=0$) and $m_- \equiv -m_+$ the corresponding negative counterpart (analogous for $n$).

\begin{enumerate}
	\item $\hat{f}^c_{m_+n_+} = +\tilde{f}^c_{m_+n_+}$
	\item $\hat{f}^s_{m_+n_+} = +\tilde{f}^s_{m_+n_+}$
	\item $\hat{f}^c_{m_-n_+} = +\hat{f}^c_{m_+n_-} = +\tilde{f}^c_{m_+n_-}$
	\item $\hat{f}^s_{m_-n_+} = -\hat{f}^s_{m_+n_-} = -\tilde{f}^s_{m_+n_-}$
\end{enumerate}
