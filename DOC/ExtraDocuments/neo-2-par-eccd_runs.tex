...
\begin{itemize}
  ...
  \item run h5merge.x from libneo to collect data (Sec.~\ref{running_running})
...
Note that \vv{hdf5tools} is provided by \vv{libneo}, while
\vv{MyMPILib} is now part of the \vv{NEO-2} repository. This keeps the
shared field and HDF5 code in \vv{libneo}, while the MPI scheduler code
stays with its sole downstream consumer.
...
setting of LIBNEO\_PATH has been changed to
\begin{verbatim}
set(LIBNEO_PATH /marconi/home/userexternal/rbuchhol/Programs/libneo/
...
\begin{verbatim}
  elseif (CMAKE_Fortran_COMPILER_VERSION VERSION_EQUAL 7.3.0)
    ...
    set(NEO2_Libs ${LIBNEO_PATH}/Build-7.3.0/ CACHE STRING "Libneo path")
\end{verbatim}
...
\vv{output} option of \vv{run}, e.g.
\begin{verbatim}
srun ... --output=out0 ...
\end{verbatim}
