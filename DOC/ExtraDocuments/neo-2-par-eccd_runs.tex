...
Instead of starting condor for each reconstruction stage of a complete NEO2 run 
manually, one can automate the process. One possible way, is to not instruct Condor
to directly run NEO2 with MPI, but specify a wrapper script that changes the input
...
\begin{itemize}
	\item \vv{condor_submit condor_submit_all_recontructions}
		\begin{itemize}
			\item NEO2-all.sh runs in surface s10000000m1 on computer A
				\begin{itemize}
					...
					\item NEO2-all.sh stops on computer A and job is finished
				\end{itemize}
			\item NEO2-all.sh runs in surface s20000000m1 on computer B
				\begin{itemize}
...
The shellscript NEO2-all.sh can be found in the Shellscript folder of NEO2. However, when you wanna
use it with condor as described above, you have to make a HARD copy of it into the \vv{temp} folder
e.g. your run folder, as Condor can only access scripts that are located in the non private regions
of the file system (your home directory \vv{~} and its subdirectories can for example NOT be accessed).

In following we provide the complete \vv{condor_submit_all_reconstructions} file
\begin{verbatim}
	...
	Log        = ../scan.log
	run_as_owner = true
	notify_user  = your.email@adresse
	notification = Error
	nice_user    = false
...
The distribution of the jobs (i.e. the execution of NEO2-all.sh) is done using the regular expression in the Queue section: In all folders in the current directory that start with `s` Condor starts an instance of NEO2-all.sh. The above example assumes that said
NEO2-all.sh lies in the run directory (see the '\vv{./}'). NOTE that '\vv{arguments}' and '\vv{requested_cpus}' have to be the same.
The file can be also found in DOC.
