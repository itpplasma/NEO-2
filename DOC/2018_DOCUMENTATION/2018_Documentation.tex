...
\vv{z_eff} have been set correctly.

The Python script has an output representing the directory names that
have been created. If you used the htcondor approach with
\vv{condor-all.submit}, then the directory names should be copied into a
file named \vv{jobs_list.txt}.
...
processors available per node, but two points should be taken into
account. First two processors should give no speedup compared to one
processor. Second, higher numbers can restrict the number of jobs that
can run at the same time on machines with large amounts of memory. If
this is good or bad depends on what you want.
...
Changing the value can be done with the script \vv{}, or with a python3
function
\begin{verbatim}
from scan_nonhdf5_tools import set_neo2in_reconstruction
set_neo2in_reconstruction(folder='./', subfolder_pattern='s[123456789].*', backup=False, value=1)
\end{verbatim}
...
\end{verbatim}
needs to be run. This code merges the output file of NEO-2 \vv{final.h5}
...
There is also a python3 funtion as alternative
\begin{verbatim}
import hdf5tools
...
\vv{$BOOTSTRAP} is short for \vv{/proj/plasma/DOCUMENTS/PHD_Gernot/2017_BOOTSTRAP/}.

\begin{table}[h]
\centering
\begin{tabular}{ll}
name/path   & comment \\
\$INTERFACE/Create\_Surfaces/Build/create\_surfaces.x & \\
\$INTERFACE/h5merge/Build/h5merge.x & \\
...
\$BOOTSTRAP/create\_dirs\_paramvalue.sh & \\
NEO2-all.sh &
\end{tabular}
...
\begin{tabular}{ll}
name/path   & comment \\
\$INTERFACE/Create\_Surfaces/create\_surf.py & similar to create\_dirs\_paramvalue.sh
\end{tabular}
...
plot\_g\_xfix.m & \\
plot\_levels\_pgf.m & \\
\$BOOTSTRAP/plot\_evolve\_h5.m & \\
\$BOOTSTRAP/count\_maxima\_bootstrap.m & \\
\$BOOTSTRAP/boozer\_rational.m & \\
\$BOOTSTRAP/plot\_levels.m &
\end{tabular}
