...
libraries.
However CMake can not take cae of building different projects in correct
order. Thus remember that you first have to build \vv{libneo} and then
\vv{neo-2}.
Another advantage is that the source- and build-directories can be
separated from each other, and both codes require that this is done.
Thus, in order to build the source code it is necessary to create a
build-directory.
The main configuration file is called \verb|CMakeLists.txt|. For reasons
...
settings for paths are in another file, \vv{ProjectConfig.cmake.in}.
...
\vv{neo-2} depends on a number of external libraries. While during the last
...
the group. As already mentioned, \vv{libneo} is also available from
github.
...
\begin{itemize}
 ...
 HDF5 files of that run. Note that for the parallelized stellarator version only
...
\vv{neo-2} is copied from the project directory. Please note that in the
...
\vv{final.h5} had been produced with a \vv{neo-2} run.
...
Here three tables listing executables, shell scripts, python scripts and octave/matlab scripts,
...
\begin{itemize}
 ...
 does will load a hdf5 file and convert it to a structure.
