...
!
  ...
  logical :: opnd
  integer :: iunit=721, iunit_p=722, iunit_m=723
  integer :: i,m,nprop,iprop,iphi,iphi0,isw_gnorm,isw_axisymm
!
  double precision :: theta,phi,alam,collpar,eps,convfac,travis_convfac
  double precision :: correct_spf
  double precision, dimension(:), allocatable :: phibounds,dummy,thebounds
!
  isw_gnorm=2
  isw_axisymm=0
  inquire(file='NEO2_DATA/gfunc_neo2.inp',exist=opnd)
  if(opnd) then
    open(iunit,file='NEO2_DATA/gfunc_neo2.inp')
    read(iunit,*) isw_gnorm
    read(iunit,*,err=10) isw_axisymm
10  close(iunit)
  endif
  if(isw_axisymm.ne.0.and.isw_axisymm.ne.1) then
    print *,'wrong input for isw_axisymm, isw_axisymm = ',isw_axisymm
    stop
  endif
!
  if(isw_axisymm.eq.0) then
    ...
    do
      nprop=nprop+1
      write(propname,*) nprop
      inquire(file='NEO2_DATA/sizeplot_etalev.'                  &
              //trim(adjustl(propname))//'.dat',exist=opnd)
      if(.not.opnd) exit
      open(iunit,file='NEO2_DATA/sizeplot_etalev.'               &
           //trim(adjustl(propname))//'.dat')
      read(iunit,*) lag,nplp1,nphi
      close(iunit)
      if(allocated(phi_mfl)) then
        allocate(dummy(nprop-1))
        dummy=phibounds
        deallocate(phi_mfl,phibounds)
      endif
      allocate(phi_mfl(nphi),phibounds(nprop))
      if(allocated(dummy)) then
        phibounds(1:nprop-1)=dummy
        deallocate(dummy)
      endif
      OPEN(iunit,file='NEO2_DATA/phi_mesh.'                      &
           //trim(adjustl(propname))//'.dat')
      do iphi=1,nphi
        read(iunit,*) phi_mfl(iphi)
      enddo
      close(iunit)
      if(nprop.eq.2) phibounds(1)=phi_mfl(1)
      phibounds(nprop)=phi_mfl(nphi)
    enddo
    deallocate(phi_mfl)
    nprop=nprop-1
    allocate(thebounds(nprop))
    thebounds=360.d0*(phibounds-phibounds(1))/(phibounds(nprop)-phibounds(1))
    print *,'Set theta:  0 < theta < 360 in degrees, counted from the outer side'
    print *,'theta?'
    read *,theta
    eps=100.d0*epsilon(1.d0)
    if(theta.le.-eps.or.theta.gt.360.d0+eps) then
      print *,'theta out of range'
      stop
    endif
    theta=theta-180.d0
    if(theta.le.0.d0) theta=theta+360.d0
    theta=max(0.d0,theta)+eps
    theta=min(360.d0,theta)-eps
    iprop=1
    do i=1,nprop
      iprop=iprop+1
      if(theta.le.thebounds(iprop)) exit
    enddo
    phi=phibounds(iprop-1)+(theta-thebounds(iprop-1)) &
       *(phibounds(iprop)-phibounds(iprop-1))         &
       /(thebounds(iprop)-thebounds(iprop-1))
    phi=max(phi,phibounds(iprop-1))
    phi=min(phi,phibounds(iprop))
  else
    print *,'Set theta:  0 < theta < 360 in degrees, counted from the outer side'
    print *,'theta?'
    read *,theta
...
!
  write(propname,*) iprop
  open(iunit,file='NEO2_DATA/sizeplot_etalev.'                &
       //trim(adjustl(propname))//'.dat')
  read(iunit,*) lag,nplp1,nphi,collpar,travis_convfac
  allocate(eta(0:nplp1))
  read(iunit,*) eta(0:nplp1)
  close(iunit)
!
  convfac=1.d0
  if(isw_gnorm.eq.3) convfac=travis_convfac
!
  allocate(phi_mfl(nphi),npass(nphi),bhat_mfl(nphi))
  allocate(fun_p(0:lag,0:3,0:nplp1,nphi),fun_m(0:lag,0:3,0:nplp1,nphi))
  OPEN(iunit,file='NEO2_DATA/phi_mesh.'                       &
       //trim(adjustl(propname))//'.dat')
  OPEN(iunit_p,form='unformatted',file='NEO2_DATA/spitf_p.'   &
       //trim(adjustl(propname))//'.dat')
  OPEN(iunit_m,form='unformatted',file='NEO2_DATA/spitf_m.'   &
       //trim(adjustl(propname))//'.dat')
  do iphi=1,nphi
  ...
  if(isw_axisymm.eq.1) then
    phi=phi_mfl(1)+(phi_mfl(nphi)-phi_mfl(1))*theta/360.d0
    phi=max(phi,phi_mfl(1)+eps)
    phi=min(phi,phi_mfl(nphi)-eps)
    correct_spf=sum(fun_m(0,0,1:npass(1),1)+fun_p(0,0,1:npass(1),1)) &
               /dfloat(2*npass(1))
    fun_p(0,0,1:nplp1,:)=fun_p(0,0,1:nplp1,:)-correct_spf
    fun_m(0,0,1:nplp1,:)=fun_m(0,0,1:nplp1,:)-correct_spf
  endif
  collpar=collpar*3.d0*sqrt(pi)/8.d0
  collpar=collpar*convfac
  fun_p=fun_p*collpar
  fun_m=fun_m*collpar
!
...
!
  ...
  do iphi=2,nphi
    if(phi_mfl(iphi).ge.phi) then
      if(abs(phi_mfl(iphi)-phi).le.abs(phi_mfl(iphi-1)-phi)) then
        iphi0=iphi
      else
        iphi0=iphi-1
      endif
