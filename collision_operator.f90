MODULE collop
  ...
  !! --> Condor cannot access data stored in afs-directories
  !! --> Therfore make sure that a local copy of the data exists in your temp-folder
  !! Traditional behavior (access data in afs-folder)
  ! path to data
  ! CHARACTER(len=100), PUBLIC :: collop_path = '/afs/itp.tugraz.at/proj/plasma/DOCUMENTS/Neo2/data-MatrixElements/'
  !! Way to access data when using Condor
  ! path to data (= current directory)
  ! CHARACTER(len=100), PUBLIC :: collop_path = '' 
