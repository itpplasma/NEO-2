...
!
  ...
  !! Modifications by Andreas F. Martitsch (04.11.2013)
  !! --> Used to process data from extra input file
  !! --> ripple_solver_ntv_extra_input.txt
  DOUBLE COMPLEX :: bnoverb0_Val
  !! End Modifications by Andreas F. Martitsch (04.11.2013)
  DOUBLE COMPLEX,   DIMENSION(:),   ALLOCATABLE :: ttmpfact
  LOGICAL :: colltest=.FALSE.
  LOGICAL :: ttmptest=.FALSE.
!  logical :: ttmptest=.true.
  LOGICAL :: nobounceaver=.TRUE. ! Always no bounce-averaging selected, except for the case isw_lorentz=1
                                 ! (see Line ~396 (settings for isw_lorentz=1))
!  logical :: nobounceaver=.false.
  ...
  NAMELIST /ntv_extra_input/ &
       bnoverb0_Val,swHatOmegaEOrEField,hatOmegaEOrEField,hatOmegaB,m_phi
  !! End Modifications by Andreas F. Martitsch (04.11.2013)
...
!bnoverb0=(1.d0,0.d0)
! value for perturbation field amplitude read from external
! input file
...
!
!! Modification by Andreas F. Martitsch (04.11.2013)
...
!  epserr_sink=imun*3.51544617242715748d-7/collpar
  ...
  ! here is even a minus sign missing)
...
PRINT *,2.d0*SUM((phi_mfl(ibeg+1:iend)-phi_mfl(ibeg:iend-1)) &
            /(h_phi_mfl(ibeg+1:iend)*bhat_mfl(ibeg+1:iend)      &
             +h_phi_mfl(ibeg:iend-1)*bhat_mfl(ibeg:iend-1)))
!
