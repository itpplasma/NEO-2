!
  module ntv_eqmat_mod
    ...
    double complex, dimension(:), allocatable :: amat_symm,amat_asymm
...
!
  ...
  DOUBLE PRECISION, DIMENSION(:), ALLOCATABLE :: dbhat_ds,dbhat_dtheta
...
!     PAUSE 'bmod and eta written'
  ...
  !------------------------------------------------------------------------
! NTV:
  geodcu_mfl=(imun*m_phi*boozer_curr_tor                             &
...
!print *,dbcovar_theta_ds/(aiota*bcovar_theta+bcovar_phi)
!print *,bcovar_theta,bcovar_phi
!pause
dbcovar_phi_ds=0.d0
ALLOCATE(dbhat_ds(ibeg:iend),dbhat_dtheta(ibeg:iend))
ALLOCATE(bcovar_s(ibeg:iend),dbcovar_s_dtheta(ibeg:iend))
dbhat_ds=0.d0
dbhat_dtheta=dlogbdphi_mfl*bhat_mfl/aiota
bcovar_s=0.d0
...
!
  ...
  double precision, dimension(ibeg:iend)      :: h_phi_mfl
  double complex,   dimension(ibeg:iend)      :: geodcu_mfl
  ...
  double precision, dimension(:), allocatable :: h_phi_new
  double complex,   dimension(:), allocatable :: geodcu_new
!
