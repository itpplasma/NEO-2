...
SUBROUTINE ripple_solver(                                 &
  ...
  USE device_mod
  !! Modifications by Andreas F. Martitsch (12.07.2013)
  !! --> Use exact value of bmod0
  USE partpa_mod, ONLY : bmod0
  !! End Modifications by Andreas F. Martitsch (12.07.2013)
...
! collpar - the same as $\kappa$ - inverse mean-free path times 4
  ...
  USE ntv_eqmat_mod, ONLY : nz_symm,nz_asymm,nz_per_pos,nz_per_neg,            &
                            ...
                            f0_coll,f0_ttmp
USE development
USE neo_magfie_mod, ONLY : boozer_iota,boozer_curr_tor,boozer_curr_pol, &
                           boozer_psi_pr
...
!
  ...
  DOUBLE PRECISION, DIMENSION(:),   ALLOCATABLE :: bcovar_s,dbcovar_s_dtheta
  DOUBLE COMPLEX,   DIMENSION(:),   ALLOCATABLE :: bnoverb0,dbnoverb0_dtheta
  !! Modifications by Andreas F. Martitsch (30.08.2013)
  !! --> Used to process data from extra input file
  !! --> ripple_solver_ntv_extra_input.txt
  DOUBLE COMPLEX :: bnoverb0_Val
  !! End Modifications by Andreas F. Martitsch (30.08.2013)
  DOUBLE COMPLEX,   DIMENSION(:),   ALLOCATABLE :: ttmpfact
  LOGICAL :: colltest=.FALSE.
  LOGICAL :: ttmptest=.FALSE.
!  logical :: ttmptest=.true.
  !! Modifications by Andreas F. Martitsch (25.08.2013)
  !! <--> Modification by Andreas F. Martitsch (30.08.2013)
  !! Block commented out since data can be processed by extra input file
  !! Define only the switch for bounce-averaging
  LOGICAL :: nobounceaver
  !!<---> End Modifications by Andreas F. Martitsch (30.08.2013)
  !! --> Switch on/off bounce-averaging
  ! LOGICAL :: nobounceaver=.TRUE. or  .FALSE.
  ! LOGICAL :: nobounceaver=<nobounceaverToken>
  !! End Modifications by Andreas F. Martitsch (25.08.2013)  
  INTEGER :: m_phi
  !! Modifications by Andreas F. Martitsch (03.09.2013)
  !! --> Introduce namelist to process data from external input file
  !! --> ripple_solver_ntv_extra_input.txt
  INTEGER :: swHatOmegaEOrEField
  DOUBLE PRECISION :: hatOmegaEOrEField
  NAMELIST /ntv_extra_input/ &
       bnoverb0_Val,swHatOmegaEOrEField,hatOmegaEOrEField,hatOmegaB,m_phi,nobounceaver
  !! End Modifications by Andreas F. Martitsch (03.09.2013)
...
!
  !! Modifications by Andreas F. Martitsch (03.09.2013)
  !! --> Read data for specifying a certain perturbation field
  !! --> and rotation velocities from an extra input file
  ! Default settings
  bnoverb0_Val=1
  swHatOmegaEOrEField=0
  hatOmegaEOrEField=0.0d0
  hatOmegaB=0.0d0
  m_phi=3
  nobounceaver=.FALSE.
  ! Read data from input file to namelist
  OPEN(unit=300813,file='ripple_solver_ntv_extra_input.txt',status='old')
  READ(unit=300813,nml=ntv_extra_input)
  CLOSE(unit=300813)
  !! End Modifications by Andreas F. Martitsch (03.09.2013)
  !! Modifications by Andreas F. Martitsch (03.09.2013)
  !! --> If E-Field is held constant, modify hatOmegaE  dependent on collisionality
  IF (swHatOmegaEOrEField .EQ. 0) THEN
     hatOmegaE=hatOmegaEOrEField
  ELSE
     IF (swHatOmegaEOrEField .EQ. 1) THEN
        hatOmegaE=hatOmegaEOrEField*boozer_iota/conl_over_mfp
     ELSE
     END IF
  END IF
  !! End Modifications by Andreas F. Martitsch (03.09.2013)
...
!tmp:
aiota=boozer_iota
bcovar_theta=boozer_curr_tor
bcovar_phi=boozer_curr_pol
dbcovar_theta_ds=boozer_curr_tor
!print *,bcovar_theta,bcovar_phi
!pause
dbcovar_phi_ds=0.d0
...
!! Modifications by Andreas F. Martitsch (30.08.2013)
!! --> Read perturbation field amplitude bnoverb0_Val
!! --> from external input file
! value for perturbation field amplitude chosen
! for comparison with results of NEO
!bnoverb0=(5.093323333577677d-5,0.d0)
! value for perturbation field read from external
! input file
bnoverb0=bnoverb0_Val
!! End Modifications by Andreas F. Martitsch (30.08.2013)
dbnoverb0_dtheta=(0.d0,0.d0)
!! Modifications by Andreas F. Martitsch (12.07.2013)
!! --> Use exact value of bmod0
! scalefac_kG=1d-4*1.9633546399999999d0/boozer_psi_pr
scalefac_kG=1d-4*bmod0/boozer_psi_pr
!! End Modifications by Andreas F. Martitsch (12.07.2013)
!print *,surface_boozer_B00,1.9633546399999999d0
!stop
!explicit number - bmod0
!print *,'scalefac_kG = ',scalefac_kG
!
!do istep=ibeg,iend
!write(2001,*) phi_mfl(istep), -bcovar_phi/(aiota*bcovar_theta+bcovar_phi) &
!*dlogbdphi_mfl(istep)*bhat_mfl(istep)/aiota*scalefac_kG 
!enddo
!stop
!

!! Modifications by Andreas F. Martitsch (25.06.2013)
!! <--> Modification by Andreas F. Martitsch (30.08.2013)
!! Block commented out since data can be processed by extra input file
!!<---> End Modifications by Andreas F. Martitsch (30.08.2013)
!! --> Try different rotation velocities (--> strong influence on transport regime)
!! Turn off rotation velocity (observe 1/nu transport regime)
!hatOmegaE=0.d0
!hatOmegaB=0.d0
!! Strong toroidal rotation (1/nu transport regime disappears)
!hatOmegaE=1.d-2
!hatOmegaB=0.d0
!! End Modifications by Andreas F. Martitsch (25.06.2013)
!end tmp
!
...
!
          DO ipart1=1,npassing+1
            nz=nz+1
!            irow(nz)=k+ipart
!            icol(nz)=k_prev+ipart1
!            amat_sp(nz)=deleta_factor
            nz=nz+1
!            irow(nz)=k+ipart
!            icol(nz)=k+npassing+1+ipart1
!            amat_sp(nz)=deleta_factor
          ENDDO
!
        ENDDO
...
!
        ...
        amat_coll(nz_coll_beg:nz_coll)=amat_sp(nz_beg:nz)
!
        amat_sp(nz_beg:nz)=fact_pos_e(istep)*amat_sp(nz_beg:nz)
!
        nz_beg=nz+1
...
! where $\kappa$=collpar
!
  denomjac=aiota*bcovar_theta+bcovar_phi
!
  nz=nz_per_neg
!
! Co-passing: sigma=1
...
!       -dbhat_ds(istep)/(bhat_mfl(istep)*aiota) 
    a1b=(bcovar_s(istep)*dlogbdphi_mfl(istep)/denomjac                   &
       - dlogbds_mfl(istep))/aiota 
    a2b=a1b+2.d0*(dbcovar_theta_ds+dbcovar_phi_ds/aiota                  &
       -          dbcovar_s_dtheta(istep))/denomjac
!
    npassing_prev=npl(istep-1)
    npassing=npl(istep)
!
          ...
          DO kk=1,4
            DO mm=0,lag
              ...
              amat_asymm(nz)=(hatOmegaE*x1mm(m,mm)+hatOmegaB*a1b*x2mm(m,mm)) &
                            * rhs_mat_energ(kk,ipart,istep)                  &
                            + hatOmegaB*a2b*x2mm(m,mm)                       &
                            * rhs_mat_energ2(kk,ipart,istep)
            ENDDO
          ENDDO
        ENDDO
!
        DO kk=1,4
          DO mm=0,lag
            ...
            amat_asymm(nz)=0.5d0*(                                           &
                          ...
                          * rhs_mat_energ(kk,npassing+1,istep)               &
                          + hatOmegaB*a2b*x2mm(m,mm)                         &
                          * rhs_mat_energ2(kk,npassing+1,istep)              &
                                 )
          ENDDO
        ENDDO
!
        DO kk=1,4
          DO mm=0,lag
            ...
            amat_asymm(nz)=0.5d0*(                                           &
                          ...
                          * rhs_mat_energ(kk,npassing_prev+1,istep-1)        &
                          + hatOmegaB*a2b*x2mm(m,mm)                         &
                          * rhs_mat_energ2(kk,npassing_prev+1,istep-1)       &
                                 )
          ENDDO
        ENDDO
!
...
!
! NTV:
!  epserr_sink=imun*3.51544617242715748d-6/collpar
!  epserr_sink=imun*3.51544617242715748d-5/collpar
!  epserr_sink=imun*3.51544617242715748d-7/collpar
  epserr_sink=imun*3.51544617242715748d-4/collpar
 
  !! Modifications by Andreas F. Martitsch (25.08.2013)
  !! <--> Modification by Andreas F. Martitsch (30.08.2013)
  !! Block commented out since data can be processed by extra input file
  !!<---> End Modifications by Andreas F. Martitsch (30.08.2013)
  !! --> Change toroidal mode number m_phi
  !m_phi=<mPhiToken>
  !! End Modifications by Andreas F. Martitsch (25.08.2013)

  !! Modifications by Andreas F. Martitsch (30.08.2013)
  !! --> Change rotfactor according to get agreement with the write-up
  !! --> (according to email from 26.08.13)
  ! previous definition of rotfactor in the code (for historical reasons,
  ! maybe also a minus sign is missing (but this has not much
  ! consequences on the solution))
  !rotfactor=epserr_sink*m_phi/3.d0
  ! definition according to the write-up
  rotfactor=(-1.0d0)*imun*m_phi
  !! End Modifications by Andreas F. Martitsch (30.08.2013)
!
!  geodcu_forw=(imun*m_phi*boozer_curr_tor                             &
!            /(boozer_curr_pol+boozer_iota*boozer_curr_tor))*bhat_mfl &
!            *exp(imun*m_phi*phi_mfl)
PRINT *,'convertion factor = ',(scalefac_kG &
*(boozer_curr_pol+boozer_iota*boozer_curr_tor)/boozer_curr_tor)**2
...
!
  IF(nobounceaver) THEN
    geodcu_forw=geodcu_forw-scalefac_kG*(bcovar_phi/denomjac)    &
               *(dbnoverb0_dtheta+bnoverb0*(imun*m_phi*aiota     &
...
!
  IF(nobounceaver) THEN
    ...
    ttmpfact=aiota*(dbnoverb0_dtheta+imun*m_phi*aiota*bnoverb0)
!
    CALL add_f01_source
!
    DEALLOCATE(ttmpfact)
  ENDIF
istep=(ibeg+iend)/3
CALL plotsource(9000,REAL(source_vector))
!
