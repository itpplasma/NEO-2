...
!
    ...
    elseif (prop_fileformat .eq. 2) then
       ...
       ! Create unlimited arrays in NetCDF file
       ...
       call nf90_check(nf90_create('dentf_p_' // trim(adjustl(propname)) // '.nc', NF90_HDF5, ncid_dentf_p))
       call nc_define(ncid_dentf_p, 'dentf_p', NF90_DOUBLE, (/lag+1, 4, nplp1+1, NF90_UNLIMITED/), var_dentf_p_id)

       call nf90_check(nf90_create('dentf_m_' // trim(adjustl(propname)) // '.nc', NF90_HDF5, ncid_dentf_m))
       call nc_define(ncid_dentf_m, 'dentf_m', NF90_DOUBLE, (/lag+1, 4, nplp1+1, NF90_UNLIMITED/), var_dentf_m_id)

       call nf90_check(nf90_create('spitf_p_' // trim(adjustl(propname)) // '.nc', NF90_HDF5, ncid_spitf_p))
       call nc_define(ncid_spitf_p, 'spitf_p', NF90_DOUBLE, (/lag+1, 4, nplp1+1, NF90_UNLIMITED/), var_spitf_p_id)

       call nf90_check(nf90_create('spitf_m_' // trim(adjustl(propname)) // '.nc', NF90_HDF5, ncid_spitf_m))
       call nc_define(ncid_spitf_m, 'spitf_m', NF90_DOUBLE, (/lag+1, 4, nplp1+1, NF90_UNLIMITED/), var_spitf_m_id)

       call nf90_check(nf90_create('enetf_p_' // trim(adjustl(propname)) // '.nc', NF90_HDF5, ncid_enetf_p))
       call nc_define(ncid_enetf_p, 'enetf_p', NF90_DOUBLE, (/lag+1, 4, nplp1+1, NF90_UNLIMITED/), var_enetf_p_id)

       call nf90_check(nf90_create('enetf_m_' // trim(adjustl(propname)) // '.nc', NF90_HDF5, ncid_enetf_m))
    ...
    do istep=ibeg,iend
      ...
      !
      if (prop_fileformat .eq. 1) then
         call h5_append(h5id_phi_mfl,  phi_mfl(istep), icounter)
         call h5_append(h5id_bhat_mfl, bhat_mfl(istep), icounter)
         call h5_append(h5id_npassing, npassing, icounter)
      elseif (prop_fileformat .eq. 2) then
         start = icounter
