...
!
    if(isw_adjoint.eq.0) then
      ...
      weightlag(1,0:lag)=collop_A(0:lag,1)
      weightlag(2,0:lag)=collop_A(0:lag,3)
      weightlag(3,0:lag)=collop_A(0:lag,2)
      anumm(0:lag,0:lag)=collop_nu_gee(0:lag,0:lag)                           &
           +z_eff*collop_nu_ginf(0:lag,0:lag)
      denmm(0:lag,0:lag)=transpose(collop_D_gee(0:lag,0:lag))
      do l=0,leg
        ailmm(0:lag,0:lag,l)=transpose(collop_I_gee(0:lag,0:lag,l))
      enddo
      asource=0.d0
      asource(0,1)=SQRT(6.d0*pi)/4.d0
      asource(0:lag,2)=collop_C(0:lag)/sqrt(pi)
