...
elseif (prop_finaljoin_mode .eq. 2) then
   ...
   do m=0,nvel
      r  = phi_exp(m, x)
      amat(nts_l+nr,nl*m+1:nl*m+nl)=r
      amat(nts_l+nr,nts_l+nr*m+1:nts_l+nr*m+nr)=r
   enddo
   ...
   if(nvel.gt.0 .and. isw_integral .eq. 1) then
      ...
      do m=0,nvel
         r  = phi_exp(m, x)
         amat(nts_l+2*nr,nl*m+1:nl*m+nl)=r
         amat(nts_l+2*nr,nts_l+nr*m+1:nts_l+nr*m+nr)=r
      enddo

      bvec_lapack(nts_l+2*nr,:)=0.d0
   endif
   !*****************************************************************
elseif (prop_finaljoin_mode .eq. 3) then
   do m=0,nvel
     amat(nl*(m+1),:)=amat(nl*(m+1),:)-amat(nts_l+nr*(m+1),:)
     bvec_lapack(nl*(m+1),:)=bvec_lapack(nl*(m+1),:)-bvec_lapack(nts_l+nr*(m+1),:)
     amat(nts_l+nr*(m+1),:)=0.d0
     amat(nts_l+nr*(m+1),nl*m+1:nl*(m+1))=1.d0
     amat(nts_l+nr*(m+1),nts_l+nr*m+1:nts_l+nr*(m+1))=1.d0
     bvec_lapack(nts_l+nr*(m+1),:)=0.d0
   enddo
end if
